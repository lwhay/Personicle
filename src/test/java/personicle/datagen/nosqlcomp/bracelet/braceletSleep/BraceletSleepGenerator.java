...
import personicle.datagen.nosqlcomp.bracelet.braceletRawList.BraceletRawList;
import personicle.datagen.nosqlcomp.bracelet.braceletRawList.BraceletRawListAlone;
...
public class BraceletSleepGenerator {
    ...
    public static void Generator(int mc, List<UUID> AttriSet, List<String> userList) throws IOException {
        ...
        BufferedWriter bw1 = new BufferedWriter(new FileWriter("./example/BigBraceletSleep.adm"));
        BufferedWriter bw2 = new BufferedWriter(new FileWriter("./example/BraceletSleep_alone.adm"));
        BufferedWriter bw3 = new BufferedWriter(new FileWriter("./example/BraceletSleep_general.adm"));
        for (UUID device : deviceSet) {
            ...
            for (int i = 0; i < gran; i++) {
                BraceletSleep BigLog = new BraceletSleep();
                // general
                ...
                // unique
                BigLog.setDate(new DateTime(begin));
                //System.out.println(event.toJSONString());
                GeneralMeasurement gm = new GeneralMeasurement(BigLog);
                BraceletSleepAlone alone = new BraceletSleepAlone(BigLog);
